...
const TranscriptionSection = ({ transcriptionData, onTranscriptionUploaded }) => {
  ...
  const handleDeleteTranscription = async () => {
    ...
    const confirmed = window.confirm(`Are you sure you want to delete this transcription? This action cannot be undone.`);
    if (!confirmed) return;
    try {
      setDeleting(true);
      const response = await fetch(API_ENDPOINTS.deleteProjectTranscription(activeProjectId, selectedTranscriptionId), { method: 'DELETE' });
      if (response.ok) {
  ...
  const handleSaveTranscription = async () => {
    ...
    try {
      setSaving(true);
      const currentTranscription = transcriptions.find((t) => t.transcription_id === parseInt(selectedTranscriptionId));
      const response = await fetch(
        API_ENDPOINTS.updateProjectTranscription(activeProjectId, selectedTranscriptionId),
        {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ name: currentTranscription?.name || 'Edited transcription', text: editedText }),
        }
      );
      if (response.ok) {
  ...
  // Send selection with appropriate template; clean text first
  ...
  const removeOverlappingHighlights = async (transcriptionId, start, end) => {
    const overlaps = highlights.filter((h) => h.startOffset < end && h.endOffset > start);
    ...
    for (const h of overlaps) {
      ...
      if (start > h.startOffset && end < h.endOffset) {
        fragmentsToKeep.push({ transcriptionId, start: h.startOffset, end: start, color: h.color });
        fragmentsToKeep.push({ transcriptionId, start: end, end: h.endOffset, color: h.color });
        await fetch(`${API_ENDPOINTS.HIGHLIGHTS}/${h.highlight_id}`, { method: "DELETE" });
        continue;
      }
    ...
    setHighlights((prev) =>
      prev.filter((h) => !overlaps.some((o) => o.highlight_id === h.highlight_id)).concat(fragmentsToKeep)
    );
  ...
  const handleHighlightText = async () => {
    ...
    await addHighlight({ transcriptionId, start: offsets.start, end: offsets.end, color: highlightColor });
    setTimeout(() => fetchHighlights(transcriptionId), 100);
  };
  ...
  const handleDownloadTranscription = async () => {
    ...
    try {
      ...
      const response = await fetch(API_ENDPOINTS.DOWNLOAD, { method: "POST", body: downloadData });
  ...
  const mergeAndDeduplicateHighlights = (ranges) => {
    ...
    for (const current of sorted) {
      ...
      if (last && current.startOffset < last.endOffset) {
        merged[merged.length - 1] = { ...last, endOffset: Math.max(last.endOffset, current.endOffset), color: current.color };
      } else {
        merged.push(current);
      }
    }
  ...
  const renderWithHighlights = (text, ranges) => {
    ...
    if (mergedRanges.length === 0) {
      return text.split('\n').map((line, i, arr) => (
        <React.Fragment key={`line-${i}`}>{line}{i < arr.length - 1 ? <br /> : null}</React.Fragment>
      ));
    }
    ...
    for (let i = 0; i < sorted.length; i++) {
      ...
      if (end > start) parts.push({ type: 'hl', text: text.slice(start, end), color: h.color, id: h.highlight_id ?? `${start}-${end}-${h.color}-${i}` });
    ...
    const renderPiece = (piece, key) => {
      ...
      if (piece.type === 'hl') {
        ...
        return (
          <span key={key} className="highlighted-text" style={{ backgroundColor: piece.color, color: textColor, padding: '2px 4px', borderRadius: '3px' }}>
            {children}
          </span>
        );
      }
  ...
  const adjustHighlightsAfterEdit = async (transcriptionId, oldText, newText) => {
    ...
    for (const h of highlights) {
      ...
      try { await fetch(`${API_ENDPOINTS.HIGHLIGHTS}/${h.highlight_id}`, { method: "DELETE" }); } catch {}
    }
  ...
  return (
    <div className="bg-slate-800 rounded-xl shadow-md p-4 flex-1 flex flex-col min-h-0 overflow-hidden">
      {/* header / controls (unchanged layout) */}
      ...
      <div className="bg-slate-700 rounded-lg p-3 flex-1 flex flex-col min-h-0">
        {!isEditing && selectedTranscriptionId && (
          <div className="flex items-center gap-2 mb-2 pb-2 border-b border-slate-600">
              ...
              {projectHighlighters.length > 0 ? (
                projectHighlighters.map((h) => (
                  <button
                    ...
                    className={`w-4 h-4 rounded border-2 ${highlightColor === h.colour ? 'border-white' : 'border-slate-500'}`}
                    style={{ backgroundColor: h.colour }}
                    onClick={() => setHighlightColor(h.colour)}
            ...
            >
              <i className="bi bi-eraser-fill text-base"></i>
            </button>
